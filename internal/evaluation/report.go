...
<table><thead><tr><th>Variant</th><th>Cases</th><th>Hit@K</th><th>Recall@K</th><th>MRR</th><th>Answer accuracy</th><th>Failures</th><th>Mean ms</th><th>P95 ms</th><th>Retrieved tokens</th><th>Calls</th><th>Cost USD</th></tr></thead><tbody>
{{range .Rows}}<tr><td>{{.ID}}</td><td>{{.Metrics.Cases}}</td><td>{{printf "%.4f" .Metrics.HitAtK}}</td><td>{{printf "%.4f" .Metrics.RecallAtK}}</td><td>{{printf "%.4f" .Metrics.MRR}}</td><td>{{printf "%.4f" .Metrics.AnswerAccuracy}}</td><td>{{.Metrics.Failures}}</td><td>{{printf "%.2f" .Metrics.LatencyMeanMS}}</td><td>{{printf "%.2f" .Metrics.LatencyP95MS}}</td><td>{{.Metrics.RetrievedTokens}}</td><td>{{.Metrics.ProviderCalls}}</td><td>{{printf "%.6f" .Metrics.CostUSD}}</td></tr>{{end}}
</tbody></table>
{{if .Metrics.Ablations}}<h2>Paired ablations</h2><table><thead><tr><th>Pair</th><th>Recall@K delta</th><th>MRR delta</th><th>Answer accuracy delta</th><th>Mean latency delta ms</th></tr></thead><tbody>{{range .Metrics.Ablations}}<tr><td>{{.Variant}} vs {{.Baseline}}</td><td>{{printf "%+.4f" .RecallAtKDelta}}</td><td>{{printf "%+.4f" .MRRDelta}}</td><td>{{printf "%+.4f" .AnswerAccuracyDelta}}</td><td>{{printf "%+.2f" .LatencyMSDelta}}</td></tr>{{end}}</tbody></table>{{end}}
